...
const _ = l("balls", 100),
  BUFFER_SIZE = _ * 6 * Float32Array.BYTES_PER_ELEMENT,
  M = l("min_radius", 2),
  G = l("max_radius", 10),
  x = l("render", 1),
...
const T = i.createShaderModule({
  ...
  resolutionBuffer = i.createBuffer({
    size: 2 * Float32Array.BYTES_PER_ELEMENT,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
  }),
  inputBuffer = i.createBuffer({
    size: BUFFER_SIZE,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
  }),
  outputBuffer = i.createBuffer({
    size: BUFFER_SIZE,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC,
  }),
  gpuReadStagingBuffer = i.createBuffer({
    size: BUFFER_SIZE,
...
let r = new Float32Array(new ArrayBuffer(BUFFER_SIZE));
for (let e = 0; e < _; e++)
  (r[e * 6 + 0] = d(M, G)),
...
i.queue.writeBuffer(
  resolutionBuffer,
  0,
  new Float32Array([t.canvas.width, t.canvas.height])
);
for (;;) {
  performance.mark("webgpu start"), i.queue.writeBuffer(inputBuffer, 0, r);
  ...
  a.dispatchWorkgroups(o),
    a.end(),
    e.copyBufferToBuffer(outputBuffer, 0, gpuReadStagingBuffer, 0, BUFFER_SIZE);
  const n = e.finish();
  i.queue.submit([n]),
    await gpuReadStagingBuffer.mapAsync(GPUMapMode.READ, 0, BUFFER_SIZE);
  const c = gpuReadStagingBuffer.getMappedRange(0, BUFFER_SIZE).slice();
  (b = new Float32Array(c)),
    gpuReadStagingBuffer.unmap(),
